...
2. **Charge Cognitive (Sweller / Miller)** :
   ...
   - `dataLabels` : Désactiver si $N > 12$ points.
3. **Accessibilité WCAG 2.2** :
   - Contraste minimal texte / fond $\ge 4.5:1$ (calculé en luminance relative).
   - Support obligatoire de `prefers-reduced-motion`.
   - Durée d'animation $\le 800\text{ ms}$ sans aucun effet cartoon `bounce`/`elastic`.
